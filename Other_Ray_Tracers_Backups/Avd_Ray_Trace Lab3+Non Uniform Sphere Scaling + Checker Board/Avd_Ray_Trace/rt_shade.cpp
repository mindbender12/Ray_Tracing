...
float intersectCylinder(SbVec3f rayOrig,SbVec3f rayDirection,my_cylinder cylinder, int locCylinder)
 {
	 ...
	 float checkVal=0,t1=-1,t2=-1,y1=0,y2=0;
...
else if((min >= 0) && (min != 11111) && (cubeOrSphereOrCylinder == 2) )// calculate for cylinder, min = 11111 means background pixel
{
		  ...
		  //Ambient Color is constant as we have assumed and it is Ka*Ia
		  pixelColor[0] = (1-cylinder.transparency[locCylinder])*(cylinder.ambientColor[locCylinder][0]*cylinder.diffColor[locCylinder][0]);
		  pixelColor[1] = (1-cylinder.transparency[locCylinder])*(cylinder.ambientColor[locCylinder][1]*cylinder.diffColor[locCylinder][1]);
		  pixelColor[2] = (1-cylinder.transparency[locCylinder])*(cylinder.ambientColor[locCylinder][2]*cylinder.diffColor[locCylinder][2]);
  ...
  if(requiredShadow == 0) // else shadow is not required, else part of if(requiredShadow == 1)
	{
		for( int r =0; r<noOfLights; r++)
	      ...
	      if (temp > 0) // the intersection point facing light source then it get color no shadow chek is happening here
		   ...
		   specularColortemp = viewRay.dot(shadowRay);
           specularColortemp = pow(specularColortemp,50);
		   specularColor[0] = specularColor[0] + specularColortemp*(intensity[0])*(sphere.specularColor[locCylinder][0]);
	       specularColor[1] = specularColor[1] + specularColortemp*(intensity[1])*(sphere.specularColor[locCylinder][1]);
		   specularColor[2] = specularColor[2] + specularColortemp*(intensity[2])*(sphere.specularColor[locCylinder][2]);			  
    ...
    pixelColor[0] = pixelColor[0] + (1-cylinder.transparency[locCylinder])*diffuseColor*(cylinder.diffColor[locCylinder][0])+ specularColor[0]+ returnShadeColor[0]+ cylinder.transparency[locCylinder]*returnRefractColor[0];
	pixelColor[1] = pixelColor[1] + (1-cylinder.transparency[locCylinder])*diffuseColor*(cylinder.diffColor[locCylinder][1])+ specularColor[1]+ returnShadeColor[1]+ cylinder.transparency[locCylinder]*returnRefractColor[1];
    pixelColor[2] = pixelColor[2] + (1-cylinder.transparency[locCylinder])*diffuseColor*(cylinder.diffColor[locCylinder][2])+ specularColor[2]+ returnShadeColor[2]+ cylinder.transparency[locCylinder]*returnRefractColor[2];
...
return pixelColorFinal;
 ...
 SbVec3f textutreCheckerBoard(SbVec3f intersection_point)
 {
	 ...
	 A = A + 1;
	   ...
	   int jump = ((int)(A + (intersection_point[0]*2)) +(int)(A +(intersection_point[2]*2)))% 2;	   
       if (jump == 0)
       {
