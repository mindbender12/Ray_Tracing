...
#include "time.h"
 ...
 float minT=-1,k,aspectRatio,dist=10,imWidth,imHeight,noOfSphere=0,noOfCube=0,noOfCylinder=0;
 ...
 if (argc != 9)
  ...
  //{ printf("Center is: %f\t %f\t %f\t\n",sphere.centerSphere[i][0],sphere.centerSphere[i][1],sphere.centerSphere[i][2]);
  // }
		   ...
		   if(requiredDof == 0) 
		    {
			 ...
			 pixelColorFinal = pixelColorFinalSup/5;			 
...
float intersect(SbVec3f rayOrig,SbVec3f rayDirection,float radius,SbVec3f centerSphere)
 ...
 /* IntersectionQuadrics Function Definition */
 float intersectCylinder(SbVec3f rayOrig,SbVec3f rayDirection,my_cylinder cylinder, int objCode,float hmin,float hmax)
 {
	 float A,B,C,minT=-1.0;
	 float a=0,b=0,c=0,d=0,e=0,f=0,g=0,h=0,i=0,j=0;
	 float checkVal =0,t0=0,t1=0;
	 rayDirection.normalize();
	 float y1, y2;

	 if(objCode ==1) // Cylinder Intersection
	 {
		 //a=cylinder.a;
		 //b=cylinder.b;
		 //c=cylinder.c;
		 ...
		 //A = a*rayDirection[0]*rayDirection[0] + b*rayDirection[1]*rayDirection[1] + c*rayDirection[2]*rayDirection[2] + d*rayDirection[0]*rayDirection[1] 
		 //    + e*rayDirection[0]*rayDirection[2] +  f*rayDirection[1]*rayDirection[2];

		 //B = 2*a*rayOrig[0]*rayDirection[0] + 2*b*rayOrig[1]*rayDirection[1] + 2*c*rayOrig[2]*rayDirection[2] + d*(rayOrig[0]*rayDirection[0] 
		 //    + rayOrig[1]*rayDirection[1]) + e*rayOrig[0]*rayDirection[0] + f*(rayOrig[1]*rayDirection[2]	+ rayOrig[2]*rayDirection[1]) 
		 //    + g*rayDirection[0] + h*rayDirection[1] + i*rayDirection[2]; 

   //      C = a*rayOrig[0]*rayOrig[0] + b*rayOrig[1]*rayOrig[1] + c*rayOrig[2]*rayOrig[2] + d*rayOrig[0]*rayOrig[1] + e*rayOrig[0]*rayOrig[2] + 
			// f*rayOrig[1]*rayOrig[2] + g*rayOrig[0] + h*rayOrig[1] + i*rayOrig[2] + j;
		 ...
		 //if (checkVal>=0)
		 // {
			//t0 = (-B-(sqrtf(checkVal)))/(2*A);
			//t1 = (-B+(sqrtf(checkVal)))/(2*A);
   ...
   float t1 = -1, t2 = -1, t = -1;
    ...
    C = rayOrig[0] * rayOrig[0] + rayOrig[2] * rayOrig[2] -1;
...
SbVec3f Shade(int noOfSphere,SbVec3f rayOrig, SbVec3f pixelEyeDist, my_sphere sphere, my_light light, int noOfLights,int recursiondepth, int requiredReflection, int requiredShadow, int refractionRequired,int noOfCube,my_cube cube,int noOfCylinder,my_cylinder cylinder)
  {
	 ...
	 float minT=0,diffuseColor=0,ambientColor=0,specularColortemp=0,*tempT,*tempCylinderT,min=11111,temp1=0,diffuseColorSub=0,*tempCubeT,minSphere=11111,minCube=11111,minCylinder=11111;
	 ...
	 if(noOfSphere>0)
	   {
			  //sphere wise loop
			   ...
			   {
				/********************************* Adding for Instancing objects Lab4 Implementation ************************************************/
				/************************************************************************************************************************************/	
				...
				/*************************** Addition Ends for Instancing objects Lab4 Implementation ************************************************/
				/************************************************************************************************************************************/
				   
				/* Calling the Intersect Function for Intersection Calculation */ 
				   tempT[q] = intersect(rayO,Raydist,sphere.radius[q],sphere.centerSphere[q]); // change rayOrig with rayO and pixelEyedist with Raydist for Instancing
	   ...
	   if(noOfCylinder > 0) 
	   {
               ...
               /********************************* Adding for Instancing objects Lab4 Implementation ************************************************/
				...
				/* Calling the intersectCylinder Function for Intersection Calculation */ 
				   tempCylinderT[q] = intersectCylinder(rayOrig,pixelEyeDist,cylinder,1,cylinder.hmin[q],cylinder.hmax[q]);
...
else if((min >= 0) && (min != 11111) && (cubeOrSphereorCylinder == 2) )// Calculate for cylinder Here
	{
		///* For Instancing Forming new ray orig and direction*/
		//  interSectionPointLocal = rayO + (Raydist)*min; // in object space 	  		  
		//  intersectionNormalLocal = (interSectionPointLocal-cylinder.centerCylinder[locCylinder]); // in object space		  
...
return pixelColorFinal;
 ...
 SbVec3f textutreCheckerBoard(SbVec3f intersection_point)
 {
	 ...
	 A = A + 1;
       ...
       int jump = ((int)(A + (intersection_point[0]*8)) + (int)(A + (intersection_point[2]*8)))% 2;	   
       if (jump == 0)
       {
