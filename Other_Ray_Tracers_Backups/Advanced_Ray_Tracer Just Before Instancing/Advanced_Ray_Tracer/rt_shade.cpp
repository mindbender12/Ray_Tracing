...
#include "time.h"
 ...
 if (argc != 9)
  ...
  {
	  printf("Transformation matrix for sphere: %d\n",i);
	  for(int j=0;j<4;j++)
	  {
		  for(int k=0;k<4;k++)
		  {
...
/*Random No generator */
 SbVec3f returnRandom()
 {
	 float i = sin(rand()/float(RAND_MAX+1));
     float j = cos(rand()/float(RAND_MAX+1)); 
	 float k = sin(rand()/float(RAND_MAX+1))*cos(rand()/float(RAND_MAX+1));

	 float a = i*sin(j/k);
	 float b = j*sin(k/i);
	 float c = k*sin(i/j);

	 return SbVec3f(a,b,c);                          
