...
#include "my_cylinder.h"
 ...
 void usage_error();
 SbVec3f Shade(int,SbVec3f,SbVec3f, my_sphere, my_light,int,int,int,int,int,int,my_cube,int,my_cylinder);
	 ...
	 for(int j=0;j<XRes;j++)
	  {
		...
		SbVec3f pointAimed = rayOrig + mc.focalLength*pixelEyeDist;// focalLength is the focal distance.
		    ...
		    if(requiredDof ==0) 
		        {
		          pixelColorFinal = Shade(noOfSphere,rayOrig,pixelEyeDist,sphere,light,noOfLights,1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
					  ...
					  for(int di=0; di<10; di++)
					   {
							...
							dofRayDir.normalize();
							pixelColorFinal = Shade(noOfSphere,dofRayOrig,dofRayDir,sphere,light,noOfLights,1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
...
float intersectCylinder(SbVec3f rayOrig,SbVec3f rayDirection,my_cylinder cylinder, int locCylinder)
 {
	 //cylinder equation as my cylinder is along to y axis
     //x^2 + z^2 -1 = 0
	...
	//cout<<"cylinder.hmin[locCylinder] "<<cylinder.hmin[locCylinder] <<endl;
	//cout<<"cylinder.hmax[locCylinder] "<<cylinder.hmax[locCylinder] <<endl;
	...
	checkVal= (B * B) - (4* A* C);
		...
		else
		{
			...
			if (y1>cylinder.hmin[locCylinder] && y1<cylinder.hmax[locCylinder])
				{
				minT = t1;
				}
			if (y2>cylinder.hmin[locCylinder] && y2<cylinder.hmax[locCylinder])
			   {
               minT= t2;
               }
			if(y1>cylinder.hmin[locCylinder] && y1<cylinder.hmax[locCylinder] && y2>cylinder.hmin[locCylinder] && y2<cylinder.hmax[locCylinder])
			{
              if(t1<t2)
			        {
                  if(t1>0)
			...
			if(y1 >= cylinder.hmin[locCylinder] && cylinder.hmin[locCylinder] >= y2 || y2 >= cylinder.hmin[locCylinder] && cylinder.hmin[locCylinder] >= y1)
			{
            t3 = (cylinder.hmin[locCylinder] - rayOrig[1]) / rayDirection[1];
            minT = t3;
             } 
			if(y1 >= cylinder.hmax[locCylinder] && cylinder.hmax[locCylinder] >= y2 || y2 >= cylinder.hmax[locCylinder] && cylinder.hmax[locCylinder] >= y1)
			{
            t3 = (cylinder.hmax[locCylinder] - rayOrig[1]) / rayDirection[1];
            minT = t3;
            }

		}

	//cout<<"minCylinderT"<<minT<<endl;
...
SbVec3f Shade(int noOfSphere,SbVec3f rayOrig, SbVec3f pixelEyeDist, my_sphere sphere, my_light light, int noOfLights,int recursiondepth, int requiredReflection, int requiredShadow, int refractionRequired,int noOfCube,my_cube cube,int noOfCylinder,my_cylinder cylinder)
  {
	 float R=0.6,G=0.6,B=0.6,temp,checkshadow=-1;
	 ...
	 int loc=0,intersection1=0,intersection2=0,intersectLight=0,locCube=0,locCylinder=0;
	 int MAXRECURSION=3,cubeOrSphereOrCylinder=0;
	 float shine = 0.9,trans=1, Ni = 0.77,epsilon = 0.00009,epsilon1= -0.000009,epsilonReflectionSphere= 0.00009,temp2=0;// epsilon1 is for refraction of cubes
...
if((min >= 0) && (min != 11111) && (cubeOrSphereOrCylinder == 0) )// calculate for sphere, min = 11111 means background pixel
{
		   ...
		   if(sphere.S[loc][0][0] == sphere.S[loc][1][1] == sphere.S[loc][2][2])
			{
				  ...
				  /* World Space Calculation Ends*/
						...
						//for (int p = 0;p<5;p++)
						//{
						//	dx = dx + (rand()/float(RAND_MAX+1));
						//	dy = dy + (rand()/float(RAND_MAX+1));
						//	dz = dz + (rand()/float(RAND_MAX+1));
						//}

						//dx = dx/5; dy = dy/5; dz = dz/5;
						//intersection_point[0] = intersection_point[0] + dx;
						//intersection_point[1] = intersection_point[1] + dy;
						//intersection_point[2] = intersection_point[2] + dz;
						//intersection_normal = (intersection_point-sphere.centerSphere[loc]);
						//intersection_normal.normalize();
 ...
 if(requiredShadow == 1) 
 {
		 ...
		 {    
			if(requiredReflection == 1) // Add reflection only if required. Determined through runtime binary parameter requiredReflection (1,0)
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
		         ...
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + reflectionRay*epsilonReflectionSphere),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
				...
				if(refractionRequired ==1)
				{
				 if(sphere.transparency[loc]>0)
				 {
			    	...
			    	if(temp2 <= 0) //(IncomingRay.Normal < 0) Ray is coming from air to object 
					{
						...
						returnRefractColor = returnRefractColor + Shade(noOfSphere,(intersection_point + (pixelEyeDist*epsilonRefractSphere)),TransmittedRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
					...
					else
					{
						...
						else
						{
                            ...
                            reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				            reflectionRay.normalize();
		                    returnRefractColor = returnRefractColor + Shade(noOfSphere,(intersection_point + reflectionRay*epsilonRefractSphere),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
	  ...
	  if(recursiondepth < MAXRECURSION) // raflection should still happen even the point is in shadow.. 
		      {    
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
		         temp1 = pixelEyeDist.dot(intersection_normal);
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionSphere),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
				 returnShadeColor = sphere.shininess[loc]*returnShadeColor;
				...
				if(refractionRequired ==1)
				{
				 if(sphere.transparency[loc]>0)
				 {
			    	...
			    	if(temp2 < 0) //(ViewRay.Normal < 0) Ray is coming from air to object 
					{
	                    ...
	                    rootContent = sqrtf(rootContent);
		                TransmittedRay = (Ni * (intersection_normal.dot(-pixelEyeDistNormalized)) - rootContent) * intersection_normal - ( Ni * -pixelEyeDistNormalized );
	                    returnRefractColor = returnRefractColor + Shade(noOfSphere,(intersection_point + (TransmittedRay*epsilonRefractSphere)),TransmittedRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
	...
	/*  pixelColor[0] = pixelColor[0] + (1-sphere.transparency[loc])*checkerBoard[0]+ specularColor[0]+ (1-sphere.transparency[loc])*returnShadeColor[0]+ sphere.transparency[loc]*returnRefractColor[0];
	  pixelColor[1] = pixelColor[1] + (1-sphere.transparency[loc])*checkerBoard[1]+ specularColor[1]+ (1-sphere.transparency[loc])*returnShadeColor[1]+ sphere.transparency[loc]*returnRefractColor[1];
      pixelColor[2] = pixelColor[2] + (1-sphere.transparency[loc])*checkerBoard[2]+ specularColor[2]+ (1-sphere.transparency[loc])*returnShadeColor[2]+ sphere.transparency[loc]*returnRefractColor[2];*/
 ...
 if(requiredShadow == 1) 
		 ...
		 {    
			if(requiredReflection == 1) // Add reflection only if required. Determined through runtime binary parameter requiredReflection (1,0)
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				 reflectionRay.normalize();
		         returnShadeColor = returnShadeColor + shine*Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionCube),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
				...
				if(refractionRequired ==1)
				{
				 if(cube.transparency[locCube]>0)
				 {
			    	...
			    	if(temp2 < 0) //(IncomingRay.Normal < 0) Ray is coming from air to object 
					{
	                    ...
	                    rootContent = sqrtf(rootContent);
		                TransmittedRay = (Ni * (intersection_normal.dot(-pixelEyeDistNormalized)) - rootContent) * intersection_normal - ( Ni * -pixelEyeDistNormalized );
	                    returnRefractColor = returnRefractColor + Shade(noOfSphere,(intersection_point + (pixelEyeDist*(epsilon1))),TransmittedRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
					...
					else
					{
						...
						else
						{
                            ...
                            reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				            reflectionRay.normalize();
		                    returnRefractColor = returnRefractColor + cube.shininess[locCube]*Shade(noOfSphere,(intersection_point + reflectionRay*epsilon1),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
				 ...
				 if(cube.transparency[locCube]>0)
				 {
					...
					else
					{
						...
						else
						{
                            ...
                            reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				            reflectionRay.normalize();
		                    returnRefractColor = returnRefractColor + shine*Shade(noOfSphere,(intersection_point + reflectionRay*epsilon1),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
	...
	pixelColor[0] = pixelColor[0] + checkerBoard[0]+ specularColor[0]+ (1-sphere.transparency[loc])*returnShadeColor[0]+ returnRefractColor[0];
	pixelColor[1] = pixelColor[1] + checkerBoard[1]+ specularColor[1]+ (1-sphere.transparency[loc])*returnShadeColor[1]+ returnRefractColor[1];
    pixelColor[2] = pixelColor[2] + checkerBoard[2]+ specularColor[2]+ (1-sphere.transparency[loc])*returnShadeColor[2]+ returnRefractColor[2];
	}
	else
	{
	...
	pixelColor[0] = pixelColor[0] + checkerBoard[0]+ specularColor[0]+ returnShadeColor[0]+ returnRefractColor[0];
    pixelColor[1] = pixelColor[1] + checkerBoard[1]+ specularColor[1]+ returnShadeColor[1]+ returnRefractColor[1];
    pixelColor[2] = pixelColor[2] + checkerBoard[2]+ specularColor[2]+ returnShadeColor[2]+ returnRefractColor[2];
	}
...
else if((min >= 0) && (min != 11111) && (cubeOrSphereOrCylinder == 2) )// calculate for cylinder, min = 11111 means background pixel
{
	      ...
	      /*For Instancing Forming new ray orig and direction*/
		  interSectionPointLocal = rayO + (Raydist)*min; // in object space 	  		  
		  intersectionNormalLocal = (interSectionPointLocal-SbVec3f(0,interSectionPointLocal[1],0)); // in object space	
		  intersectionNormalLocal.normalize();
		  cylinder.F[locCylinder].multVecMatrix(interSectionPointLocal, intersection_point);
		  cylinder.FinvT[locCylinder].multDirMatrix(intersectionNormalLocal,intersection_normal);

		   SbVec3f checkerBoard = textutreCheckerBoard(intersection_point);
  ...
  if(requiredShadow == 0) // else shadow is not required, else part of if(requiredShadow == 1)
	  ...
	  if(recursiondepth < MAXRECURSION) // raflection should still happen even the point is in shadow.. 
		      {    
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionSphere),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
				 returnShadeColor = cylinder.shininess[locCylinder]*returnShadeColor;
		         	 
				  }

				if(refractionRequired ==1)
				{
				 if(sphere.transparency[locCylinder]>0)
				 {
					 temp2 = pixelEyeDistNormalized.dot(intersection_normal); 
   ...
   pixelColor[0] = pixelColor[0] + diffuseColor*(cylinder.diffColor[locCylinder][0])+ specularColor[0]+ returnShadeColor[0]+ cylinder.transparency[locCylinder]*returnRefractColor[0];
	pixelColor[1] = pixelColor[1] + diffuseColor*(cylinder.diffColor[locCylinder][1])+ specularColor[1]+ returnShadeColor[1]+ cylinder.transparency[locCylinder]*returnRefractColor[1];
    pixelColor[2] = pixelColor[2] + diffuseColor*(cylinder.diffColor[locCylinder][2])+ specularColor[2]+ returnShadeColor[2]+ cylinder.transparency[locCylinder]*returnRefractColor[2];

    /*pixelColor[0] = pixelColor[0] + checkerBoard[0]*(cylinder.diffColor[locCylinder][0])+ specularColor[0]+ returnShadeColor[0]+ cylinder.transparency[locCylinder]*returnRefractColor[0];
	pixelColor[1] = pixelColor[1] + checkerBoard[1]*(cylinder.diffColor[locCylinder][1])+ specularColor[1]+ returnShadeColor[1]+ cylinder.transparency[locCylinder]*returnRefractColor[1];
    pixelColor[2] = pixelColor[2] + checkerBoard[2]*(cylinder.diffColor[locCylinder][2])+ specularColor[2]+ returnShadeColor[2]+ cylinder.transparency[locCylinder]*returnRefractColor[2];*/
...
SbVec3f textutreCheckerBoard(SbVec3f intersection_point)
 {
	 ...
	 A = A + 1;
	   ...
	   int jump = ((int)(A + (intersection_point[0]*2)) +(int)(A +(intersection_point[1]*2)))% 2;	   
       if (jump == 0)
       {
	 ...
	 //2. Texture Type Smoothly varying color generator
	  /*Let fract(x) = x - floor(x)
      (r, g, b) = (1 - |2*fract(x) - 1|, 1-|2*fract(y) - 1|,
      1-|2*fract(z) - 1|)*/

	 /* Color[0] = (1- (abs(2*(intersection_point[0]-floor(intersection_point[0])))-1));
      Color[1] = (1- (abs(2*(intersection_point[1]-floor(intersection_point[1])))-1));
      Color[2] = (1- (abs(2*(intersection_point[2]-floor(intersection_point[2])))-1));
