...
#include "time.h"
 ...
 SbVec3f refract(SbVec3f,SbVec3f,float,float);
 ...
 int main(int argc, char **argv)
{
 ...
 int XRes,YRes,requiredReflection=1,requiredShadow=1,refractionRequired=1,requiredSupSample=1;
 FILE *fp;
	 ...
	 for(int j=0;j<XRes;j++)
	  {
		...
		//pixelColorFinal = 255*pixelColorFinal;				
			...
			pixelColorFinal[0] = pixelColorFinalSup[0]/16;
			pixelColorFinal[1] = pixelColorFinalSup[1]/16;
			pixelColorFinal[2] = pixelColorFinalSup[2]/16;
...
// Refract function Definition
 SbVec3f refract(SbVec3f pixelEyeDist,SbVec3f intersection_normal,float Ni,float ifChange)
 {
	 SbVec3f TransmittedRay(0,0,0);
	 float NV=0,rootContent=0,temp1=0;
	 pixelEyeDist.normalize();
	 pixelEyeDist = (-1)*pixelEyeDist;  //Vital part     

	 if(ifChange <0)
	 {
       rootContent = (1 - Ni * Ni * (1 - (intersection_normal.dot(pixelEyeDist) * (intersection_normal.dot(pixelEyeDist)))));
	   if(rootContent>0.0)
	   {
		   rootContent = sqrtf(rootContent);
		   TransmittedRay = (Ni * (intersection_normal.dot(pixelEyeDist)) - rootContent) * intersection_normal - ( Ni * pixelEyeDist );
	   }
	 }
