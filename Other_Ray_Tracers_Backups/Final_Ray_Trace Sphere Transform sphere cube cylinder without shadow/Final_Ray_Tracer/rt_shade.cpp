...
/* Creating Dynamic array for red pixels */
Red = (float **) malloc(XRes*sizeof(float));
for(int i=0;i<XRes;i++)
Red[i] = (float *) malloc(YRes*sizeof(float));

/*creating Dynamic array for green pixels */
Green = (float **) malloc(XRes*sizeof(float));
for(int i=0;i<XRes;i++)
Green[i] = (float *) malloc(YRes*sizeof(float));

/*creating Dynamic array for blue pixels */
Blue = (float **) malloc(XRes*sizeof(float));
for(int i=0;i<XRes;i++)
Blue[i] = (float *) malloc(YRes*sizeof(float));
 ...
 Picture p;
  ...
  //Storing Pixel Values in Color Matrix
  ...
  {   
    for (int j=0;j<XRes;j++) 
	{
	  ...
	  Red[i][j]=(float)r/255;
	  Green[i][j]=(float)g/255;
	  Blue[i][j]=(float)b/255;
	}
  }

  //Just for printing the pixel values from input jpeg file to see it works 
	  ...
	  if(recursiondepth < MAXRECURSION) // raflection should still happen even the point is in shadow.. 
		      {    
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				 reflectionRay.normalize();
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionCube),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
		         }
...
else if((min >= 0) && (min != 11111) && (cubeOrSphereOrCylinder == 2) )// calculate for cylinder, min = 11111 means background pixel
{
		  ...
		  intersection_point = rayOrig + (pixelEyeDist)*min;
		  intersection_normal = SbVec3f(2*intersection_point[0],0,2*intersection_point[2]);
		  intersection_normal.normalize(); */

		   interSectionPointLocal = rayO + (Raydist)*min; // in object space 	  		  
		  //intersectionNormalLocal = (interSectionPointLocal-SbVec3f(0,interSectionPointLocal[1],0)); // in object space wrong Normal calculation
		  intersectionNormalLocal = SbVec3f(2*interSectionPointLocal[0],0,2*interSectionPointLocal[2]);
		...
		{    
			if(requiredReflection == 1) // Add reflection only if required. Determined through runtime binary parameter requiredReflection (1,0)
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				 reflectionRay.normalize();
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point - reflectionRay*epsilonCylinderReflection),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
  ...
  if(requiredShadow == 0) // else shadow is not required, else part of if(requiredShadow == 1)
	{
	  ...
	  if(recursiondepth < MAXRECURSION) // raflection should still happen even the point is in shadow.. 
		      {    
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
				 reflectionRay.normalize();
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point - pixelEyeDist*epsilonCylinderReflection),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube,noOfCylinder, cylinder); 
				 }
...
SbVec3f textutreCheckerBoard(SbVec3f intersection_point)
 {
	 ...
	 A = A + 1;
	   ...
	   int jump = ((int)(A + (intersection_point[0]*1.5)) +(int)(A +(intersection_point[2]*1.5)))% 2;	   
       if (jump == 0)
       {
