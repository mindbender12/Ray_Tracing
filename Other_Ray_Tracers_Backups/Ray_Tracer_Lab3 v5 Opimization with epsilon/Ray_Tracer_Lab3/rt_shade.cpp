	 ...
	 for(int j=0;j<XRes;j++)
	  {
		...
		{	
		  SbVec3f pointAimed = rayOrig + focalLength*pixelEyeDist;
		   if(requiredDof == 0) 
		    {
			 for(int k =0; k<16; k++)
			 {
		   ...
		   else // Implement DOF + Super Sample here
		   {
					...
					for(int k =0; k<16; k++)// supersample loop
			          {

					  for(int di=0; di<16; di++)// DOF loop
					   {
			                float dk = rand()/float(RAND_MAX+1);
                            float dl = rand()/float(RAND_MAX+1);                            
					  ...
					  pixelColorFinalSuperSample += pixelColorTemp/16;
					  pixelColorTemp.setValue(0,0,0);
			  	      
			   }
			   pixelColorFinal = pixelColorFinalSuperSample/16;
			   pixelColorFinal = 255*pixelColorFinal;
...
/* Definition of getNormalCube which returns the normal for each intersection point in cube */
 SbVec3f getNormalCube (SbVec3f point,int locCube,my_cube cube)
 {
	...
	if (abs(point[0] - cube.pointFront[locCube][0]) <= EPS) return SbVec3f(-1,0,0);
	else if (abs(point[0] - cube.pointBack[locCube][0]) <= EPS) return SbVec3f(1,0,0);
	else if (abs(point[1] - cube.pointFront[locCube][1]) <= EPS) return SbVec3f(0,-1,0);
	else if (abs(point[1] - cube.pointBack[locCube][1]) <= EPS) return SbVec3f(0,1,0);
	else if (abs(point[2] - cube.pointFront[locCube][2]) <= EPS) return SbVec3f(0,0,-1);
	else if (abs(point[2] - cube.pointBack[locCube][2]) <= EPS) return SbVec3f(0,0,1);
...
SbVec3f Shade(int noOfSphere,SbVec3f rayOrig, SbVec3f pixelEyeDist, my_sphere sphere, my_light light, int noOfLights,int recursiondepth, int requiredReflection, int requiredShadow, int refractionRequired,int noOfCube,my_cube cube)
  {
	 ...
	 int loc=0,intersection=0, intersection1=0,intersection2=0,sp,intersectLight=0,locCube=0;
	 int MAXRECURSION=2,cubeOrSphere=0;
	 float shine = 0.5,trans=1, Ni = 1/1.5,epsilon = 0.00009,epsilon1= -0.000009,epsilonReflectionSphere= 0.000009,temp2=0;// epsilon1 is for refraction
	 pixelEyeDistNormalized = pixelEyeDist;
	 pixelEyeDistNormalized.normalize();
	 float diffuselight=0,DiamLight=4;
	 float epsilonRefractSphere = 0.00001;
		   ...
		   if (temp > 0) // the intersection point facing light source
	       {
			...
			{								 
				if(cube.transparency[q] <=0)
				   {
				/* Calling the Intersect Function for Intersection calculation */ 
				checkshadow = intersect((shadowRayOrig + 0.0000007*shadowRaydir),shadowRaydir,sphere.radius[q],sphere.centerSphere[q]);
				...
				for(int q=0;q<noOfCube;q++)
				  {
					 if(cube.transparency[q] <=0)
				   {
					...
					/* Calling the Intersect Function for Intersection calculation */ 
					checkshadow = intersectCube((shadowRayOrig - 0.000007*shadowRaydir),shadowRaydir,cube.pointFront[q],cube.pointBack[q]);
					if (checkshadow == -1.0 )
					  {
                      ...
                      for(int q=0;q<noOfSphere;q++)
                       {
				          if(sphere.transparency[q]<=0)
				           {
			                  /* Calling the Intersect Function for Intersection calculation */ 
	                           checkshadow = intersect((shadowRayOrig + 0.0007*softShadowRayDir),softShadowRayDir,sphere.radius[q],sphere.centerSphere[q]);
					...
					for(int q=0;q<noOfCube;q++)
					  {
						 ...
						 {
						/* Calling the Intersect Function for Intersection calculation */ 
						checkshadow = intersectCube((shadowRayOrig - 0.0007*softShadowRayDir),softShadowRayDir,cube.pointFront[q],cube.pointBack[q]);
						if (checkshadow == -1.0 )
						  {
						  intersection2 = 0; // no intersection
						  }
	...
	if(recursiondepth < MAXRECURSION) //raflection should still happen even the point is in shadow.. 
		 {    
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionSphere),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube); 
		         returnShadeColor[0] = sphere.shininess[loc]*returnShadeColor[0];
		         returnShadeColor[1] = sphere.shininess[loc]*returnShadeColor[1];
		         returnShadeColor[2] = sphere.shininess[loc]*returnShadeColor[2];
 ...
 /********************************************** This part is for cube calculation *****************************/
 else if((min >= 0) && (min != 11111) && (cubeOrSphere ==1) )// calculate for cube, intersection point is on cube
	{
          ...
          //Ambient Color is constant as we have assumed and it is Ka*Ia
		  pixelColor[0] = (1-cube.transparency[locCube])*(cube.ambientColor[locCube][0]*cube.diffColor[locCube][0]);
		  pixelColor[1] = (1-cube.transparency[locCube])*(cube.ambientColor[locCube][1]*cube.diffColor[locCube][1]);
		  pixelColor[2] = (1-cube.transparency[locCube])*(cube.ambientColor[locCube][2]*cube.diffColor[locCube][2]);
            ...
            for(int q=0;q<noOfSphere;q++)
              {
				 ...
				 {
			    /* Calling the Intersect Function for Intersection calculation */ 
	            checkshadow = intersect((shadowRayOrig + 0.0007*shadowRaydir),shadowRaydir,sphere.radius[q],sphere.centerSphere[q]);
				if (checkshadow == -1.0 )
				  {
				  intersection1 = 0; // no intersection
				  }				
			...
			for(int q=0;q<noOfCube;q++)
              {
			    if(cube.transparency[q]<= 0)
				 {
			    /* Calling the Intersect Function for Intersection calculation */ 
	            checkshadow = intersectCube((shadowRayOrig - 0.0007*shadowRaydir),shadowRaydir,cube.pointFront[q],cube.pointBack[q]);
				if (checkshadow == -1.0 )
				  {
				     intersection2 = 0; // no intersection
				  }				
		 ...
		 {    
			if(requiredReflection == 1) // Add reflection only if required. Determined through runtime binary parameter requiredReflection (1,0)
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
		         temp1 = pixelEyeDist.dot(intersection_normal);
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionCube),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube); 
				 returnShadeColor[0] = cube.shininess[locCube]*returnShadeColor[0];
		         returnShadeColor[1] = cube.shininess[locCube]*returnShadeColor[1];
		         returnShadeColor[2] = cube.shininess[locCube]*returnShadeColor[2];
		   ...
		   if (temp > 0) // the intersection point facing light source
	       {
			 ...
			 if(tempLight[2] >= tempLight[0] && tempLight[2] >= tempLight[1])
			  ...
			  for(int k=0;k<numShadowRays;k++)
			  {
					...
					SbVec3f subPixelCenter = lightCenter  -(DiamLight/2)*uSoft - (DiamLight/2)*vSoft + DiamLight*(dx)*uSoft + DiamLight*(dy)*vSoft;// radius = 3                    
                    SbVec3f softShadowRayDir = subPixelCenter - shadowRayOrig;	
					shadowRayOrig = shadowRayOrig + 0.0007*softShadowRayDir;
					softShadowRayDir.normalize();
	...
	if(recursiondepth < MAXRECURSION) //raflection should still happen even the point is in shadow.. 
		 {    
		        ...
		        //Here Formulate Reflection Ray and Call Shade again (Recursive Call)
                 ...
                 reflectionRay = pixelEyeDist - 2*temp1*(intersection_normal);
		         returnShadeColor = returnShadeColor + Shade(noOfSphere,(intersection_point + pixelEyeDist*epsilonReflectionCube),reflectionRay,sphere,light,noOfLights,recursiondepth+1,requiredReflection,requiredShadow,refractionRequired,noOfCube,cube); 
		         returnShadeColor = cube.shininess[locCube]*returnShadeColor;	         
	  ...
	  specularColor = specularColorlight;
  		  ...
  		  pixelColor[0] = pixelColor[0] + (1-cube.transparency[locCube])*diffuseColor*(cube.diffColor[locCube][0])+ specularColor[0]+ (1-cube.transparency[locCube])*returnShadeColor[0]+ cube.transparency[locCube]*returnRefractColor[0];
		  pixelColor[1] = pixelColor[1] + (1-cube.transparency[locCube])*diffuseColor*(cube.diffColor[locCube][1])+ specularColor[1]+ (1-cube.transparency[locCube])*returnShadeColor[1]+ cube.transparency[locCube]*returnRefractColor[1];
     	  pixelColor[2] = pixelColor[2] + (1-cube.transparency[locCube])*diffuseColor*(cube.diffColor[locCube][2])+ specularColor[2]+ (1-cube.transparency[locCube])*returnShadeColor[2]+ cube.transparency[locCube]*returnRefractColor[2];
